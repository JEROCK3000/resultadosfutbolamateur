<?php /** app/Views/weekly_schedule/index.php */ ?>
...
<div class="card">
  <div class="card-header">
    <h1 class="card-title">📅 Sorteo y Programación Semanal</h1>
  </div>
  <p class="text-muted" style="margin-bottom:20px">Seleccional campeonato para programar la logística (Estadios, Fechas Exactas y Árbitros) de las jornadas generadas en el Fixture Base.</p>
  ...
  <?php else: ?>
    <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(250px, 1fr)); gap:15px">
      <?php foreach ($leagues as $l): ?>
        <a href="<?= url("programacion/{$l['id']}") ?>" style="text-decoration:none; color:inherit">
          <div style="padding:20px; border:1px solid var(--color-border); border-radius:12px; background:var(--color-surface); transition:transform 0.2s, box-shadow 0.2s; cursor:pointer" onmouseover="this.style.transform='translateY(-3px)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.1)'" onmouseout="this.style.transform='none'; this.style.boxShadow='none'">
            ...
            <div style="margin-top:15px; text-align:right">
              <span class="badge badge-success">Programar Jornada →</span>
            </div>
          </div>
        </a>
      <?php endforeach; ?>
